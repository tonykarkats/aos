...
/**
 *  \brief Recursively searches the tree and returns a memory chunk which can hold the requested size.
 *  \brief tree The paging tree.
 *  \params x The returned node.
 *  \params bytes The size requested.
 */
rb_red_blk_node* find_memory_chunk (rb_red_blk_tree* tree, rb_red_blk_node* x, size_t bytes, rb_red_blk_node* guard)
{
    ...
    if (x != tree->nil) {
        ...
        init_guard = find_memory_chunk(tree, x->right, bytes, init_guard);
        return init_guard;
   }
   return init_guard;
...
/**
 * \brief  This function finds a memory chunk that can hold 'bytes' bytes
...
lvaddr_t allocate_memory(rb_red_blk_tree* tree, size_t bytes)
{
    ...
    rb_red_blk_node* node = find_memory_chunk(tree, tree->root->left, bytes, NULL);
