	...
	size_t msg_words, size_t *words_sent)
{
	...
	if (words_sent) { *words_sent = msg_words; }
		return lmp_chan_send1(c, LMP_FLAG_SYNC | LMP_FLAG_YIELD, NULL_CAP,
								buf[0]);
}
*/
...
//	lmp_chan_init(channel);
	//uintptr_t msgbuf[1] = { 1};
	//size_t words_sent;

	/*
	channel.remote_cap = cap_initep;	

	err = send_message(&channel, msgbuf, 1, &words_sent);
	if (err_is_fail(err)) {
	...
	*/
	...
	err = lmp_ep_send2(cap_initep, LMP_FLAG_SYNC, NULL_CAP, 1, 'a');
	if (err_is_fail(err)) {
